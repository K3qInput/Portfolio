              ...
              <div className="max-w-lg w-full">
                {servicePackages.filter(pkg => pkg.id === 10).map((pkg, index) => (
                  ...
                  >
                    ...
                    <h3 className="text-xl font-bold text-center mb-3 text-slate-300">{pkg.title}</h3>
                    <p className="text-slate-400 text-center mb-4 text-sm">{pkg.description}</p>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {servicePackages.filter(pkg => pkg.category === "monthly" && pkg.popular).map((pkg, index) => (
              <motion.div
                ...
                transition={{ delay: 0.1 * index }}
                className={`glass-effect p-6 rounded-2xl relative transition-all duration-300 cursor-pointer hover:shadow-3xl h-[500px] flex flex-col ${
              ...
              >
                {pkg.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
                    <Badge className="bg-gradient-to-r from-green-400 to-green-500 text-slate-900 px-6 py-2 font-bold shadow-xl border-2 border-green-300 animate-pulse">
                      <Star className="w-4 h-4 mr-2 fill-slate-900 text-slate-900" />
                      Most Popular
                    </Badge>
                  </div>
                )}
